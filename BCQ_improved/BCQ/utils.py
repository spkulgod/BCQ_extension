...
class ReplayBuffer(object):
	...
	# Expects tuples of (state, next_state, action, reward, not_done)
	def add(self, data):
	...
	def sample(self, batch_size):
		...
		return (np.array(state), 
			...
			np.array(not_done).reshape(-1, 1))

	def save(self, filename):
		np.save("./buffers/"+filename+".npy", self.storage)

	def load(self, filename):
		self.storage = np.load("../buffer_generate/"+filename+".npy", allow_pickle=True)